...
# Make species counts table for Kraken2 
k2rep_counts <- dcast(brep.m, sample ~ name, sum, value.var = "kraken_assigned_reads")
row.names(k2rep_counts) <- k2rep_counts$sample
k2rep_counts$sample <- NULL
k2rep_counts[is.na(k2rep_counts)] <- 0
row.names(k2rep_counts) <- paste0(row.names(k2rep_counts), "_k2")
dim(k2rep_counts) # 8 5366

# Convert to Relative Abundance
k2rep_pct <- decostand(k2rep_counts, method = "total")
...
all_samples_comp.m <- rbind(melt(as.matrix(syn_pct)), melt(as.matrix(k2rep_pct)), melt(as.matrix(brep_pct)), 
                            melt(as.matrix(gan_pct)), melt(as.matrix(cent_pct)), zymo_pct)
...
# Set factor levels
same_samp_dist_synth$type2 <- factor(same_samp_dist_synth$type2, levels = c("kraken2", "bracken", "ganon", "centrifuge"))
```
...
```{r}
# Plot
ggplot(same_samp_dist_synth, aes(type2, dist)) + geom_boxplot(lwd=0.5) + scale_y_continuous(limits = c(0.50,0.70)) + 
  geom_jitter(height = 0, width = 0.2, size=5, alpha=0.6, aes(col=syn_type, shape=even)) + 
  stat_compare_means(label.x.npc = 0.2, col="firebrick4") + 
  labs(col="Syn. Metagenome Type", x="", y="Bray-Curtis Dissimilarities\n", shape="Composition", title = "Accuracy Comparison: Synthetic Metagenomes") + 
...
# Set colors
posctrl_colors = colorRampPalette(brewer.pal(12, "Set3"))
...
ggplot(sample1234_pct_zym.m, aes(name, pct)) + geom_bar(stat="identity", aes(fill=species), lwd=0.1, width = 0.75, color="black") + 
  facet_grid(~sample, scales="free_x", space="free_x") + scale_fill_manual(values=c(posctrl_colors(ncol(sample1234_pct_zym)-1), "grey50")) + 
  labs(x="", y ="Relative Abundance%\n", fill="", title = "Taxonomic Comparison: MMC Samples") + 
  scale_y_continuous(expand=c(0,0.01)) + guides(fill=guide_legend(ncol=1)) + theme_bw() +
...
ggplot(resource_comp.m, aes(tool, value)) + geom_bar(stat = "identity", position = "dodge", aes(fill=tool)) + facet_wrap(~variable, scales="free_y")
